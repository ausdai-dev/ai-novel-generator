...
"""
    ...
    def expand_content(self):
        ...
        try:
            ...
            if not chapters:
                QMessageBox.warning(self, "章节识别失败", "无法从大纲中识别出章节，请检查大纲格式")
                return
                
            total_chapters = len(chapters)
            total_words = self.word_count_slider.value()
            target_words_per_chapter = total_words // total_chapters
            ...
            for i, chapter in enumerate(chapters):
                ...
                # 提取预计字数
                expected_words = target_words_per_chapter
                ...
                # 允许的字数误差范围（20%）
                word_margin = int(expected_words * 0.2)
...
3. 添加生动的细节描写和人物对话
4. 丰富环境描写和人物心理
5. 扩写后的内容应在{min_words}-{max_words}字之间
...
"""
                ...
                for retry in range(max_retries + 1):
                    ...
                    if response:
                        ...
                        # 检查字数
                        content_length = len(response.replace('\n', ''))
                        if min_words <= content_length <= max_words:
                            # 字数符合要求
                            saved_content = self._save_chapter_content(response, i, chapter_title, "扩写")
                            if saved_content:
                                self.expanded_chapters.append(saved_content)
                                break
                        elif retry < max_retries:
                            # 字数不符合要求，添加更严格的提示重试
                            if content_length < min_words:
                                prompt += f"\n\n当前扩写字数为{content_length}字，比最低要求少{min_words - content_length}字，请适当增加细节描写和对话内容。"
                            else:
                                prompt += f"\n\n当前扩写字数为{content_length}字，比最高限制多{content_length - max_words}字，请适当精简内容。"
                            continue
                        else:
                            QMessageBox.warning(self, "扩写提示", 
                                f"第{i+1}章扩写字数（{content_length}字）不在目标范围内（{min_words}-{max_words}字），" +
                                "但仍会保存结果。建议后续手动调整。")
                            saved_content = self._save_chapter_content(response, i, chapter_title, "扩写")
                            if saved_content:
                                self.expanded_chapters.append(saved_content)
                    
                    if not response and retry == max_retries:
                        QMessageBox.warning(self, "扩写失败", f"第{i+1}章扩写失败，请检查网络连接或重试")
                        break

            # 处理扩写后的章节
            self._process_expanded_chapters(total_chapters)
    ...
    def _clean_response(self, response):
        """清理响应中的统计信息和总结"""
        # 移除包含特定关键词的行及其后面的内容
        keywords = ['实际字数', '字数统计', '优化总结', '### 优化总结', '总字数']
